...
class Theme implements BaseLib{
	...
	public function __construct( )
	{
		...
		//获取模板路径位置
		$this->theme_path = THEME_ADMIN;
		$this->theme_admin_path = THEME_PC;
	 ...
	 */
	public function  theme_get_theme_by_path( $is_pc = THEME_PC)
	{
		$result = array( 
			'data' => array(),
			'status' => 0,
		...
		{
			case THEME_ADMIN:
				$theme_path = $apppath.THEME."/admin";
					break;
			case THEME_MOBILE:
				$theme_path = $apppath.THEME."/mobile";
					break;
			default:
				$theme_path = $apppath.THEME."/pc";
					break;
		}
	        ...
	        while ( false !== ($file = readdir ( $handle )) ) 
	        {
	            ...
	            if ($file != "." && $file != ".."&&!strpos($file,".")) 
	            {
	                $themes[$i] = array(
	                			...
	                			"screenshot"=>"/".$theme_path."/".$file."/theme.png",
	                	);
	                $i++;
	            }
            }
